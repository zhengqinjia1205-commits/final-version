...
最终裁决以 **hold-out RMSE 最小** 为核心。

当存在高级模型时：

- 若 \(\mathrm{RMSE}_{adv} < 0.95 \times \mathrm{RMSE}_{baseline}\) 且残差通过 Ljung–Box 白噪声检验（\(p>0.05\)），则选高级模型
- 否则选表现最好的基线模型，并给出“高级模型未表现出显著统计优势”的解释

当样本量 \(n<30\)：跳过高级模型，只在 ETS/Naïve 中择优。
